  ...
  useEffect(() => {
    ...
    const unsubscribeTrades = onValue(tradesRef, (snapshot) => {
      ...
      if (data) {
        ...
        const totalProfit = tradesArray.filter(trade => trade.status === 'closed')
...
// ONLY REPLACE THIS SPECIFIC useEffect HOOK in Dashboard.tsx
...
useEffect(() => {
  ...
  for (const trade of openTrades) {
    ...
    console.log(`Trade: ${trade.tradeName}, Profit: ${profit.toFixed(2)}`);
  }
  ...
  console.log(`Total Initial ZAR: ${totalInitialZAR.toFixed(2)}, Total Profit ZAR: ${totalProfitZAR.toFixed(2)}, Percentage: ${profitPercentage.toFixed(2)}%`);
  
  // Update state with total profit from all open trades
  setCurrentProfit({ 
